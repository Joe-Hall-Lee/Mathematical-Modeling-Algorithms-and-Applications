...
f = simplify(f) % 化简目标函数
f1 = diff(f, x1), f2 = diff(f, x2) % 求目标函数关于 x1、x2 的偏导数
[x10, x20] = solve(f1, f2) % 求驻点
subplot(121), fplot(x10, [0.002, 0.02]), title('') % 画 x1 关于 a 的曲线
xlabel('$a$', 'Interpreter', 'Latex')
ylabel('$x_1$', 'Interpreter', 'Latex', 'Rotation', 0)
subplot(122), fplot(x20, [0.002, 0.02]), title('') % 画 x2 关于 a 的曲线
xlabel('$a$', 'Interpreter', 'Latex')
...
sx2a = dx20 * 0.01 / 7043
F = subs(f, {x1, x2}, {x10, x20}) % 求关于 a 的目标函数
