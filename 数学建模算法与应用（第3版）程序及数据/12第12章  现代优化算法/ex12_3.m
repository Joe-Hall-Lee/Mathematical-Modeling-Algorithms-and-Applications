...
d = d + d';
w = 50;
g = 100; % w 为种群的个数，g 为进化的代数
rand('state', sum(clock)); % 初始化随机数发生器
for k = 1:w % 通过改良圈算法选取初始种群
    c = randperm(100); % 产生 1, …, 100 的一个全排列
    c1 = [1, c + 1, 102]; % 生成初始解
...
for k = 1:g % 该层循环进行遗传算法的操作
    ...
    while ~length(by)
        by = find(rand(1, w) < 0.05); % 产生变异操作的地址
    end
    num1 = length(by);
    B = J(by, :); % 产生变异操作的初始染色体
    ch2 = rand; % 产生混沌序列的初始值
    ...
    for j = 1:num1
        bw = sort(2+floor(100*rand(1, 2))); % 产生变异操作的 2 个地址
        B(j, bw) = ch2([j, j + 1]); % bw 处的两个基因发生了变异
    end
    G = [J; A; B]; % 父代和子代种群合在一起
    [SG, ind1] = sort(G, 2); % 把染色体翻译成 1, …, 102 的序列 ind1
    num2 = size(G, 1);
    long = zeros(1, num2); % 路径长度的初始值
    for j = 1:num2
        for i = 1:101
